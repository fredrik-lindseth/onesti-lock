...
- **set_pin capacity check**: when the lock has reported `NumberOfPINUsersSupported` (50 on both NimlyPRO and NimlyCodePRO), `set_pin` rejects slots at or above it (`pin_rules.py`). Before the attribute has been read (sleepy lock at setup, or a variant without the attribute) the manual's 3-999 range applies. Whether slots >= 50 actually work on real hardware is still unverified; see the capacity test below.
