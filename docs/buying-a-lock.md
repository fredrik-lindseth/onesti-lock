...
One thing is not firmware and cannot be fixed: the radio sits inside a metal lock body at the outermost point of the house, with no antenna sticking out anywhere. The signal is weak, and the lock cannot relay for anything, so it lives off whatever mains-powered device happens to be nearest. Budget for putting one of those near the door on the inside, and for an afternoon of moving things around. Without it the lock is unreliable or will not pair at all, and a battery-powered repeater does not count. [docs/debugging.md](debugging.md#signal-issues) has the measurements and what to do about them, and [docs/community-reports.md](community-reports.md#range-and-coverage) what other owners went through.
