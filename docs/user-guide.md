...
9. **Going back to 1.4.0 or older duplicates the entities.** 1.5.0 moved the device and the entities off the lock's Zigbee address and onto the config entry, and an older version registers them all again under the old key: two devices, two of every sensor, and your entity ids on the half that is no longer updated. Upgrading to 1.5.0 again removes the duplicates and puts the entity ids back on the sensors that are updated, so that is the cleanup; deleting the entry is not needed and would cost the slot names and the PIN status.
