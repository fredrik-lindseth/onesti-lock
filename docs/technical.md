...
Each lock gets one device of ours, built in `entity.py` by `build_device_info()`: the model read off ZHA, `serial_number` set to the IEEE address, and a name made of the model and the last four characters of the address, so two locks of the same model are told apart on the device page and in entity ids.

Identifiers are `(DOMAIN, entry_id)`, not the IEEE address. A replaced Connect Module brings a new address for the same door, and keying on the entry lets the reconfigure flow point the entry at the new module while the device, its entities, their names and whatever a dashboard refers to stay put. Entity unique ids are `<entry_id>-<key>` for the same reason.

Old keys are rewritten by `_migrate_to_entry_id_keys()` in `__init__.py`, which setup calls on every load rather than the 2.3 version step calling it once. A config entry is saved a second after it changes and the registries use the 180 second delay during startup, so a version bump can reach disk three minutes before the registry write it belongs to. Driving the rewrite off the registries makes it a no-op once it is done, and self-healing when a restart, a power cut or a rollback to a release that keyed on the address left the two halves disagreeing. A key that is already taken means the row is a duplicate of one the user already has, so the stale row is removed rather than rewritten onto it, which would raise and fail the setup.

The zigbee connection ZHA uses is carried only from HA 2026.9, and which half runs is tested on both targets. There a connection is unique within one config entry, the two devices stay apart, and `build_device_info()` sets `via_device_id` to ZHA's device so ours hangs off it. Through 2026.8 a connection is unique across config entries, so the registry would fold our device into ZHA's row, and ZHA removes that row whole when the lock leaves the network or the user removes the device, our sensors with it. So no connection is sent there and the device stands alone, which is what releases up to 1.4.0 gave. `DeviceInfo` lost `via_device` in 2026.9 as well; which field it has is what the code branches on (`HAS_VIA_DEVICE_ID` in `entity.py`).
...
The three numbers are also three sensors of their own, with the unique ids `<entry_id>-pin-users`, `<entry_id>-pin-length-min` and `<entry_id>-pin-length-max`. They are `EntityCategory.DIAGNOSTIC` and `entity_registry_enabled_default = False`, so they exist in the registry but are off until someone turns them on under the device. There is no state class and no stored data: a capacity is not a measurement, and the answer already lives in `entry.options["capabilities"]`, which the sensor reads on every write. Until the lock has answered the state is `None`. They used to be attributes on the activity sensor (`num_pin_users`, `min_pin_length`, `max_pin_length`) and are not any more, which is a breaking change in 1.5.0.
