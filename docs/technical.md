...
The manifest does not name `bluetooth_adapters` yet, and that is deliberate. Home Assistant's own Bluetooth integrations depend on it, because it depends on `bluetooth` and loads after ESPHome, Shelly and the other integrations that bring remote scanners, so every proxy is known before the entry sets up. As long as nothing here imports `bluetooth.py`, naming it would set the Bluetooth stack up on every installation and hold this entry back until those four are done, without anyone getting anything for it. **The dependency goes back in the same change that first imports `bluetooth.py`**, together with the pin on the Bluetooth stack; the module's own imports (bleak, bleak-retry-connector) only exist where Bluetooth is set up. `tests/test_coordinator.py` and `tests_ha/test_bluetooth.py` both hold the manifest to this, so the change that adds the dependency has to change those two tests as well. A `bluetooth` matcher stays out for longer than that: it would start discovery flows in the UI before any flow can enroll a lock.
...
PIN commands used to fail with `IndexError: tuple index out of range` even though the command reached the lock. The source is ZHA, not the lock, and the line was found on 2026-09-20: `Device.issue_cluster_command` in zha 0.0.59 (`zha/zigbee/device.py`, the code behind the `issue_zigbee_cluster_command` service the integration used to call) checks `if response[1] is not ZclStatus.SUCCESS`, which assumes the two-field Default Response `(command_id, status)`. Set PIN Code Response and Clear PIN Code Response have one field, `status`, so index 1 does not exist. zha 2.2.2 has replaced that line with `getattr(response, "status", None)` and a comment saying exactly this, so the bug is already fixed upstream and there is nothing left to report. The vendor's own 2021 spec ([elife-module-spec.md](zigbee-protocol/elife-module-spec.md)) documents the answer as one status byte, FAILURE or SUCCESS, which is the standard one-field response zigpy parses without trouble; the error was never in the frame. `send()` reads the status by name, so it should not see the error at all. It still catches `IndexError` and treats the command as sent, logged at debug level only, in case the lock's answer itself trips zigpy's parser.
...
The radio module inside the Onesti/Nimly locks, also sold separately as an accessory. It carries Zigbee and Bluetooth LE on one Nordic part (see below); this integration uses the Zigbee side only.

| Property              | Value                                                                  |
| --------------------- | ---------------------------------------------------------------------- |
| Manufacturer code     | `0x1234` (4660), not registered with the Zigbee Alliance; it is the default an unconfigured ZBOSS stack reports, and ZBOSS is the Zigbee stack in Nordic's nRF Connect SDK |
| Max buffer size       | 108                                                                    |
...
Two independent clues point at the same silicon. The EUI64 of every Onesti Products AS lock seen so far begins with `f4:ce:36`, an OUI the IEEE registry assigns to Nordic Semiconductor ASA, and the manufacturer code 4660 is what a ZBOSS stack reports before a vendor sets its own. The only Nordic parts with an 802.15.4 radio (nRF52840, nRF52833, nRF5340) carry Bluetooth LE on the same die, so the module has a BLE radio whatever its revision; whether the firmware on a given module uses it is another question (the vendor's module guide says Bluetooth is only on "newer versions", and no advertisement has been captured from Fredrik's lock). An earlier edition of this file guessed at a TI CC2530, which has no Bluetooth and cannot be it. Which of the Nordic parts it is has not been established. Details and the Basic cluster version attributes are in [docs/connect-bridge/hardware-gateway.md](connect-bridge/hardware-gateway.md#which-silicon).

The firmware on that part has changed more than the hardware. The vendor's 2021 spec for the module lists no `0x0100` or `0x0101` attribute, and a March 2021 Zigbee sniff of an EasyCodeTouch on the same Nordic OUI shows lock state reports and nothing manufacturer-specific at all ([zigbee-captures.md](zigbee-protocol/zigbee-captures.md#captures-from-other-locks)). The operation event this integration is built on arrived in a later firmware, and which one is not known. One report ([Z2M#6551](https://github.com/Koenkk/zigbee2mqtt/issues/6551)) shows a differently-branded EasyAccess lock on a different OUI and a different `ManufacturerName`, which our config flow would not discover in the first place; see [docs/hardware-generations.md](hardware-generations.md) for that report and every other one caught so far.
