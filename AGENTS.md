...
Event listener (in __init__.py)
  ...
  ├── Updates activity sensor (skips auto-lock to preserve "Frode låste opp med kode")
...
4. **Activity sensor auto-lock suppression**: Auto-lock events fire the HA event but do NOT update the activity sensor, to avoid overwriting "Frode låste opp med kode" with "Auto-lås".
