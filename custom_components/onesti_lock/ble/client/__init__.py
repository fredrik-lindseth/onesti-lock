...
connect firmware floor and the factory credential. tracing.py is the hook
through which a caller sees every frame, and bleak_transport.py the
Transport over bleak, the one module here that needs bleak installed.

This package imports no submodule of its own; the public API is
ble/__init__.py, which leaves bleak_transport.py out.
"""
